"""
╔══════════════════════════════════════════════════════════════════╗
║   BNF + Minervini 統合バックテスト v2.8.0                        ║
║   ハイブリッド版: 翌日寄付き約定 + セクター重み付け + 3本上限    ║
║   + 米国フィルタ + ボラ過熱フィルタ強化(現実重視の最終版)        ║
╚══════════════════════════════════════════════════════════════════╝
...
SECTOR_BNF_THRESHOLDS = {
    ...
    "AI半導体":        -15.0,  # 半導体(-15%)と同じ
    "FAセンサー":      -12.0,  # 産業用ロボット(-12%)と同じ
    "計測機器":        -12.0,  # 産業用ロボット(-12%)と同じ
    "制御機器":        -12.0,  # 産業用ロボット(-12%)と同じ
    "防衛":             -8.0,  # 重工業(-8%)と同じ、政策連動で安定
    "宇宙":            -18.0,  # 高ボラ・新興テーマ
    "光学":            -15.0,  # 半導体類似(HOYAなど)
    "再エネ":          -15.0,  # 中ボラ、政策連動
    "電子機器":        -15.0,  # 電子部品(-22%)より分散
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  ★v2.7.8: セクター別リスク重み付け
#  v2.7.7 の10年バックテスト結果(セクター別勝率・平均損益)に基づく診断
#  - 強セクター: リスク1.5倍(株数1.5倍)
#  - 弱セクター: リスク0.5倍(株数半減)
#  - 除外セクター: 0倍 = エントリーしない
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

SECTOR_RISK_MULTIPLIER = {
    # 🟢 強セクター(勝率60%以上 & 平均損益 +5%以上、サンプル10件以上)
    "人材":            1.5,  # 勝率71% / +7.62%
    "ホテル":          1.5,  # 勝率92% / +7.18%
    "投資会社":        1.5,  # 勝率71% / +6.58%
    "リース":          1.5,  # 勝率87% / +6.51%
    "証券":            1.5,  # 勝率65% / +5.45%
    "半導体シリコン":  1.5,  # 勝率67% / +5.19%
    "半導体製造装置":  1.5,  # 勝率64% / +5.17%(主力)
    "建機":            1.5,  # 勝率68% / +5.03%
    "モーター":        1.5,  # 勝率71% / +4.91%

    # 🔴 除外セクター(勝率45%以下 or 平均損益マイナス、サンプル10件以上)
    "フリマEC":        0.0,  # 勝率25% / -2.22% - メルカリ
    "スポーツ":        0.0,  # 勝率36% / -1.21% - シマノ・アシックス
    "カー用品":        0.0,  # 勝率35% / -0.36% - オートバックス
    "不動産サービス":  0.0,  # 勝率70%だが平均-0.22%
    "消費者金融":      0.0,  # 勝率44% / -0.13% - アコム

    # 🟡 リスク半減(勝率44-47%の微妙ライン)
    "日用品":          0.5,  # 勝率47% / -0.63%
    "バイオ":          0.5,  # 勝率44% / +0.89%
    "半導体":          0.5,  # 勝率41% / +1.07%(製造装置じゃない方)
    "SaaS":            0.5,  # 勝率44% / +1.15%
    "産業用ロボット":  0.5,  # 勝率44% / +1.79%(意外に弱い)

    # フォールバック(リスト未掲載のセクターは 1.0倍)
    "default":         1.0,
}


def get_sector_risk_mult(sector):
    """セクター別のリスク倍率を返す(v2.7.8)"""
...
def detect_market_regime(global_data, date):
    ...
    if "^GSPC" in global_data:
        ...
        if len(df_sub) >= 200:
            ...
            # 🆕 v2.7.6: S&P500 の前日変化率(1日・3日)を計算
            if len(df_sub) >= 2:
                prev_close = df_sub["Close"].iloc[-2]
                if prev_close > 0:
                    signals["sp500_change_1d"] = (close / prev_close - 1) * 100
            if len(df_sub) >= 4:
                close_3d_ago = df_sub["Close"].iloc[-4]
                if close_3d_ago > 0:
                    signals["sp500_change_3d"] = (close / close_3d_ago - 1) * 100
...
def check_momentum_signal(df, idx, sp500_change_1d=0.0, sp500_change_3d=0.0):
    ...
    🆕 v2.7.6: 米国市場フィルタ追加
    ・S&P500 前日変化率 < -1.0% → 抑制(米国急落の翌日は手を出さない)
    ・S&P500 3日変化率 < -3.0% → 抑制(連続下落トレンドを回避)
    """
    if idx < 200:
        return False, None

    # 🆕 v2.7.6: 米国市場フィルタ(MOMENTUMの後追いリスクを軽減)
    ...
    # ★v2.8.0: しきい値を 15% → 12% に厳格化(決算サプライズ後の追っかけ防止)
    ret_5d = (close / df["Close"].iloc[idx-5] - 1) * 100 if idx >= 5 else 0
    if ret_5d > 12:
        return False, None

    # ★v2.8.0: 単日急騰(=決算サプライズ可能性)もフィルタ
    if idx >= 1:
        prev_close = df["Close"].iloc[idx-1]
        if prev_close > 0:
            day_change = (close / prev_close - 1) * 100
            if day_change > 8:  # 1日で+8%超えたら過熱(決算等)
                return False, None
...
class IntegratedBacktester:
    ...
    def backtest_stock(self, ticker, name, sector, df):
        ...
        for idx in range(200, len(df)):
            ...
            # ── エントリー判定 ──
            if not in_position:
                regime, regime_signals = detect_market_regime(self.global_data, date)
                self.regime_counts[regime] += 1
                # 🆕 v2.7.6: 米国市場の変化率(MOMENTUMフィルタ用)
                sp500_1d = regime_signals.get("sp500_change_1d", 0.0)
                sp500_3d = regime_signals.get("sp500_change_3d", 0.0)
                ...
                if is_daily:
                    ...
                    # 🆕 v2.7.6: 米国市場フィルタ追加
                    # 🆕 v2.7.9: MOMENTUM 翌日寄付き約定モード(現実値検証用)
                    momentum_next_day_entry = False  # フラグ
                    momentum_next_idx = None
                    if (selected_strategy is None and
                        ...
                        mom_ok, pivot = check_momentum_signal(df, idx,
                                                              sp500_change_1d=sp500_1d,
                                                              sp500_change_3d=sp500_3d)
                        if mom_ok:
                            # ★v2.7.9: シグナル日の翌日寄付きで約定する
                            # 実運用想定: シグナル日のEODに逆指値(pivot以上)を仕込む
                            # → 翌日寄付きが pivot 以上なら約定、未満なら未約定でトレード成立せず
                            next_idx = idx + 1
                            if next_idx < len(df):
                                next_open = df["Open"].iloc[next_idx]
                                if (not pd.isna(next_open) and next_open > 0
                                        and next_open >= pivot):
                                    # 翌日寄付きが pivot 以上 = 逆指値成立
                                    selected_strategy = "MOMENTUM"
                                    selected_entry_price = float(next_open)  # 寄付き値で約定
                                    selected_stop = float(next_open) * 0.95  # 寄付きから-5%
                                    momentum_next_day_entry = True
                                    momentum_next_idx = next_idx
                            # 翌日データなし or 寄付き<pivot ならトレード成立せず(スキップ)
                ...
                # ★v2.7.9: MOMENTUM 翌日寄付き約定の場合は entry_date を翌日に
                if momentum_next_day_entry and momentum_next_idx is not None:
                    entry_date = df.index[momentum_next_idx]
                else:
                    entry_date = date
                entry_regime = regime
                half_taken = False
                ...
                # ★v2.7.8: セクター別リスク重み付け(強1.5/弱0.5/除外0)
    ...
    def _apply_concurrent_limit(self, trades):
        """同時保有銘柄数の上限 + ★v2.8.0: 同セクター3本上限"""
        if not trades:
            return trades

        max_c = self.max_concurrent
        max_per_sector = self.config.get("max_per_sector", 3)  # ★v2.8.0
        sorted_trades = sorted(trades, key=lambda x: x["entry_date"])

        accepted = []
        sector_cap_rejected = 0
        # 各トレードに対して同時保有数と同セクター保有数をカウント
        for trade in sorted_trades:
            entry = trade["entry_date"]
            sector = trade.get("sector", "不明") or "不明"
            currently_open = [a for a in accepted
                              if a["entry_date"] <= entry < a["exit_date"]]
            if len(currently_open) >= max_c:
                continue
            # ★v2.8.0: 同セクター3本上限チェック
            same_sector_count = sum(
                1 for a in currently_open
                if (a.get("sector") or "不明") == sector
            )
            if same_sector_count >= max_per_sector:
                sector_cap_rejected += 1
                continue
            accepted.append(trade)

        # ログ表示用に保存
        self.sector_cap_rejected = sector_cap_rejected
        return accepted
    ...
    def _apply_compound_recalculation(self, trades):
        ...
        for trade in sorted_trades:
            ...
            # ★v2.7.8: 複利モードでもセクター別リスク重み付けを適用
            sector_mult = get_sector_risk_mult(trade.get("sector", ""))
            if sector_mult == 0.0:
                # 除外セクターは複利再計算でもスキップ(損益ゼロ扱い)
                continue
            risk_amount *= sector_mult
...
# ★v2.7.7: セクター別統計(MOMENTUM 主体・診断ツール)
def show_sector_breakdown(trades, min_trades=10):
    """セクター × 戦略別の勝率・平均損益・累計損益を表示
    強いセクター/弱いセクターを発見するための診断ツール
    """
    if not trades:
        return
    ...
    # セクター × 戦略 集計
    from collections import defaultdict
    sector_stats = defaultdict(lambda: defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0, "pnl_pct_sum": 0.0}))
    sector_total = defaultdict(lambda: {"count": 0, "wins": 0, "pnl": 0.0, "pnl_pct_sum": 0.0})

    for tr in trades:
        sector = tr.get("sector", "不明") or "不明"
        strategy = tr.get("strategy", "?")
        pnl = tr.get("pnl", 0) or 0
        pnl_pct = tr.get("pnl_pct", 0) or 0
        is_win = pnl > 0

        s = sector_stats[sector][strategy]
        s["count"] += 1
        s["wins"] += 1 if is_win else 0
        s["pnl"] += pnl
        s["pnl_pct_sum"] += pnl_pct

        tot = sector_total[sector]
        tot["count"] += 1
        tot["wins"] += 1 if is_win else 0
        tot["pnl"] += pnl
        tot["pnl_pct_sum"] += pnl_pct

    # ───── 1. セクター総合ランキング ─────
    t1 = Table(title=f"[bold magenta]🏆 セクター別 総合成績(全戦略)・件数{min_trades}以上[/]",
               box=box.ROUNDED, title_style="bold magenta")
    t1.add_column("セクター", style="bold")
    t1.add_column("件数", justify="right")
    t1.add_column("勝率", justify="right")
    t1.add_column("平均損益", justify="right")
    t1.add_column("累計損益", justify="right")
    t1.add_column("評価", justify="center")

    # 平均損益でソート
    ranked = sorted(
        [(sec, s) for sec, s in sector_total.items() if s["count"] >= min_trades],
        key=lambda x: x[1]["pnl_pct_sum"] / x[1]["count"], reverse=True
    )

    for sec, s in ranked:
        wr = s["wins"] / s["count"] * 100
        avg = s["pnl_pct_sum"] / s["count"]
        pnl_c = "green" if s["pnl"] > 0 else "red"
        avg_c = "green" if avg > 1.5 else "yellow" if avg > 0 else "red"
        # 評価ラベル
        if wr >= 60 and avg >= 2.0:
            evaluation = "[green]🟢 強い[/]"
        elif wr >= 50 and avg >= 0.5:
            evaluation = "[yellow]🟡 普通[/]"
        elif wr < 45 or avg < 0:
            evaluation = "[red]🔴 弱い[/]"
        else:
            evaluation = "[white]⚪ 中立[/]"
        t1.add_row(
            sec, str(s["count"]), f"{wr:.1f}%",
            f"[{avg_c}]{avg:+.2f}%[/]",
            f"[{pnl_c}]¥{s['pnl']:+,.0f}[/]",
            evaluation,
        )
    ...
    # ───── 2. MOMENTUM 戦略のセクター別成績(主力戦略の診断) ─────
    t2 = Table(title=f"[bold blue]⚔ MOMENTUM 戦略のセクター別成績(件数{min_trades}以上)[/]",
               box=box.ROUNDED, title_style="bold blue")
    t2.add_column("セクター", style="bold")
    t2.add_column("件数", justify="right")
    t2.add_column("勝率", justify="right")
    t2.add_column("平均損益", justify="right")
    t2.add_column("累計損益", justify="right")

    mom_ranked = sorted(
        [(sec, ss["MOMENTUM"]) for sec, ss in sector_stats.items()
         if ss.get("MOMENTUM", {}).get("count", 0) >= min_trades],
        key=lambda x: x[1]["pnl_pct_sum"] / max(1, x[1]["count"]), reverse=True
    )
    for sec, s in mom_ranked:
        wr = s["wins"] / s["count"] * 100
        avg = s["pnl_pct_sum"] / s["count"]
        pnl_c = "green" if s["pnl"] > 0 else "red"
        avg_c = "green" if avg > 1.5 else "yellow" if avg > 0 else "red"
    ...
    # ───── 3. 推奨除外/強化リスト ─────
    weak = [(sec, s) for sec, s in sector_total.items()
            if s["count"] >= min_trades and (s["wins"] / s["count"] < 0.45 or s["pnl_pct_sum"] / s["count"] < 0)]
    strong = [(sec, s) for sec, s in sector_total.items()
              if s["count"] >= min_trades and s["wins"] / s["count"] >= 0.60 and s["pnl_pct_sum"] / s["count"] >= 2.0]

    if weak or strong:
        from rich.panel import Panel
        msg_lines = []
        if strong:
            strong_sorted = sorted(strong, key=lambda x: x[1]["pnl_pct_sum"]/x[1]["count"], reverse=True)
            msg_lines.append("[bold green]🟢 強いセクター(重み付け増 or 集中検討):[/]")
            for sec, s in strong_sorted[:10]:
                wr = s["wins"] / s["count"] * 100
                avg = s["pnl_pct_sum"] / s["count"]
                msg_lines.append(f"  ・{sec}: 勝率{wr:.0f}% / 平均{avg:+.2f}% / {s['count']}件")
        if weak:
            weak_sorted = sorted(weak, key=lambda x: x[1]["pnl_pct_sum"]/x[1]["count"])
            msg_lines.append("\n[bold red]🔴 弱いセクター(除外検討):[/]")
            for sec, s in weak_sorted[:10]:
                wr = s["wins"] / s["count"] * 100
                avg = s["pnl_pct_sum"] / s["count"]
                msg_lines.append(f"  ・{sec}: 勝率{wr:.0f}% / 平均{avg:+.2f}% / {s['count']}件")
        console.print(Panel("\n".join(msg_lines), title="🎯 セクター診断結果", border_style="bright_yellow"))
...
def main():
    ...
    # ★v2.7.7: セクター別診断
    show_sector_breakdown(trades, min_trades=10)
