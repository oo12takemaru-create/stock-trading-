"""
================================================================================
 デイリースキャナー v2.8.1 (341銘柄・セクター重み付け・米国フィルタ統合版)
 BNF + Minervini + MOMENTUM 統合戦略 - 実運用シグナル生成ツール

 ★v2.8.1 (2026-08-13): MOMENTUMに新高値型フィルタ追加(書籍24『ロケット投資』検証より)
   終値が52週高値の95%以上のときのみシグナル。10年BTで勝率48.8→51.9% /
   PF 2.05→2.25 / 最大DD -25.6→-18.9%。詳細 = v2_10_rocket_verification_report.md
================================================================================
...
def check_momentum_signal(df, sp500_change_1d=0.0, sp500_change_3d=0.0):
    ...
    # ★v2.8.1: 新高値型フィルタ(書籍24『ロケット投資』検証で採用)
    #   52週高値の近くから飛ぶ機体は押しが浅く、-5%損切りに掛かりにくい
    high_52w = df["High"].iloc[max(0, idx-252):idx+1].max()
    if pd.isna(high_52w) or high_52w <= 0 or close < high_52w * 0.95:
        return False, None
