...
- 書籍の検証も統計値のみ。**銘柄名・銘柄コード・銘柄バスケットは返さない**。元データに併存する明細(連想の `baskets.csv` 499行・決算の `events_raw.csv`・自社株買いの `buyback_*.csv` 5,523行)は**生成側でも開いていない**
...
`docs/indicator_verdict.json` / `docs/etf_decay.json` / `docs/earnings_drift.json` /
`docs/supply_demand_events.json` の7つは
