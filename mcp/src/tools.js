...
async function getDailySignals(args, { load }) {
  ...
  return meta({
    ...
    // ★全銘柄ベースの該当数は配信されていない★
    // 生成側(scanner_free.py)は全銘柄を判定したあと `rows[:top_n]` で上位3件に絞り、
    // 全体の該当数は捨てている。上位3件の中での内訳を返すと
    // 「今日は何件あったか」の答えとして読まれてしまうので、返さない。
    counts: null,
    counts_note:
      "この日に公開ルールへ該当した全銘柄の数は、配信しているデータに含まれていない。" +
      "上位3件の中での内訳は「今日は何件あったか」の答えにならないため返さない。" +
      "本番システム(3戦略統合)の当日シグナル件数は full_system_today.signal_count にある",
    full_system_today: {
    ...
    how_to_read: [
      ...
      "ルールに該当した全銘柄の数は配信されていないため counts は null。" +
        "件数が要る場合は full_system_today.signal_count(本番システム3戦略の当日合計)を見る",
...
async function listToolsGuide() {
  return meta({
    ...
    limits: [
      "★銘柄を返すのは1件だけ★ 公開ルールに該当した全銘柄の一覧は配布しない",
      "その日の該当件数(全銘柄ベース)は配信データに無いため返せない。本番システムの当日シグナル件数だけは返す",
    ...
    how_to_read: [
      ...
      "銘柄を返すのは1件だけ。該当した全銘柄の一覧は配布しない。全銘柄ベースの該当件数は配信データに無いため counts は null になる",
