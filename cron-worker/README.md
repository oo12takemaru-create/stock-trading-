...
21:00  pipeline-daily.yml  ← {"backup":"true"}
23:30  data-healthcheck.yml
合計 46 回 / 13 種類
```

日付を変えると土曜・祝日も見られる。`ran=` を付けると「18:00 が既に成功している日」を再現でき、
21:00 の予備が止まることを確かめられる。
...
```json
{
  ...
  "secrets": { "GH_PAT": true, "RESEND_API_KEY": true, "NOTIFY_TO": true },
...
無料枠の範囲。cron の発火が1日76回、そこから起動する dispatch が平日46回。
